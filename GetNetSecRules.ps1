...
## Functions to collect details on each different policy type (Quarantine, Application, Isolation and VDI)
 ...
 function GetQuarRules {
  ...
  Write-Host "######################"
  Write-Host ""
  Write-Host "Policy Type: Quarantine"
  Write-Host ""
  ...
  Write-Host ""
    ...
    if ($r.status.description -like "*Default*") {
      Write-Host ""
      Write-Host "--------COMPLETE VM ISOLATION--------"
      Write-Host "NO INBOUND/OUTBOUND TRAFFIC IS PERMITTED"
    }else {
...
function GetAppRules {
  ...
  Write-Host "######################"
  Write-Host ""
  Write-Host "Policy Type: Application"
  Write-Host ""
  write-host "Name:" $r.status.name
  write-host "Description:" $r.status.description
  Write-Host ""

    if ($apprule.action -eq "APPLY") {
      $state = "ENFORCED"
    }else {
      $state = "MONITORING"
    }
    ...
    foreach ($sg in $apprule.inbound_allow_list) {
      ...
      if ($sg.address_group_inclusion_list.kind -eq "address_group"){
        ...
        if ($sg.protocol -eq "TCP") {
          ...
          elseif ($sg.protocol -eq "ICMP") {
            Write-Host "Address Group:" $agn ": ICMP"
          }
        }
...
function GetIsoRules {
  ...
  Write-Host "######################"
  Write-Host ""
  Write-Host "Policy Type: Isolation"
  Write-Host ""
  write-host "Name:" $r.status.name
  write-host "Description:" $r.status.description
  Write-Host ""
...
function GetVDIRules {

  Write-Host ""
  Write-Host "######################"
  Write-Host ""
  Write-Host "Policy Type: VDI/AD Rule"
  Write-Host ""
  write-host "Name:" $r.status.name
  write-host "Description:" $r.status.description
  Write-Host ""

    if ($vdirule.action -eq "APPLY") {
      $state = "ENFORCED"
    }else {
      $state = "MONITORING"
    }
  ...
  $tg = $vdirule.target_group.filter.params
  Write-Host ""
  Write-Host "------APPLIES TO AD GROUP------"
  foreach ($t in $tg) {
    $param = Get-Member -InputObject $t -membertype noteproperty 
    foreach ($x in $param.name) {
      Write-Host $tg.$x
    ...
    Write-Host "---- INBOUND SOURCE(S)------"
    foreach ($sg in $vdirule.inbound_allow_list) {
      Write-Host "RuleID:" $sg.rule_id
      Write-Host "SOURCE : ALLOWED"
    ...
    Write-Host "---- OUTBOUND TARGETS(S)------"
    foreach ($sg in $vdirule.outbound_allow_list) {
      Write-Host "RuleID:" $sg.rule_id
      Write-Host "TARGET : ALLOWED"
...
######## Main script function #### 
  ...
  foreach ($r in $NRlist.entities) {  # $r is each rule entity with each policy 
    ...
    if ($a.name -eq "quarantine_rule") {
      ...
      elseif ($a.name -eq "isolation_rule") {
        $isorule = $r.spec.resources.isolation_rule
        GetIsoRules
        }
      elseif ($a.name -eq "ad_rule") {
        $vdirule = $r.spec.resources.ad_rule
        GetVDIRules
      }
    }
}
